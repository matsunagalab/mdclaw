...
def _task_agent_prompt(task_id: str, instruction_file: Path) -> str:
    ...
    return (
        ...
        "categorize the benchmark suite, or write benchmark-wide solver scripts. "
        "Task-local helper scripts are allowed only when they execute real "
        "workflow steps for this task and are recorded in provenance.command_log.\n\n"
        "Use the mdclaw conda environment for Python helpers, e.g. "
        "`conda run -n mdclaw python ...`; do not rely on the system `python3` "
        "for OpenMM, gemmi, or MDClaw imports.\n\n"
        "Do not remove or hand-edit DAG node directories, progress.json, or "
        "node.json. If a workflow step must be retried, create a new node or "
        "use the MDClaw node/need tools so provenance remains auditable.\n\n"
