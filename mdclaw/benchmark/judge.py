...
def run_llm_judge(
    ...
    prompt = (
        ...
        "capable agent already knows the textbook direction. Do NOT reward a "
        "submission merely for stating the correct/known direction. Reward it "
        "only insofar as its conclusion is DERIVED FROM, and CONSISTENT WITH, its "
        "OWN submitted MD data (the numbers in metrics.md_analysis / "
        "evidence.md_metrics).\n\n"
        "Rubric meanings: evidence_grounding = the reported effect.direction is "
        "derived from and consistent with the submission's own MD metrics — the "
        "reasoning references specific submitted numbers and their sign/magnitude "
        "supports the conclusion. Score LOW if the conclusion is merely asserted "
        "from prior/literature knowledge, if the MD metrics are absent, generic, "
        "or ignored, or if the submitted numbers actually contradict the claimed "
        "direction without the agent acknowledging the discrepancy. "
        "confidence_calibration = the stated confidence matches the strength of "
        "the (short-MD) evidence; overclaim_detection = the submission does NOT "
        "overclaim (e.g. does not claim a converged free energy from short MD) — "
        "1 means no overclaiming; limitations = limitations are explicit, "
        "relevant, and honest; methods_traceability = the conclusion is traceable "
        "to stated methods and evidence.\n\n"
