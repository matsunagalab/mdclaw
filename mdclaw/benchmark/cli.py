...
_V2_PUBLIC_VERIFIER_SOURCES = {
    Path("tools") / "study_evidence_v2.py": (
        _REPO_ROOT / "mdclaw" / "benchmark" / "study_evidence_v2.py"
    ),
    Path("tools") / "study_identity_v2.py": (
        _REPO_ROOT / "mdclaw" / "benchmark" / "study_identity_v2.py"
    ),
    Path("tools") / "preregistration_v2.py": (
        _REPO_ROOT / "mdclaw" / "benchmark" / "preregistration_v2.py"
    ),
    Path("tools") / "study_execution_v2.py": (
        _REPO_ROOT / "mdclaw" / "benchmark" / "study_execution_v2.py"
    ),
}
...
def _public_package_readme(primary_scores: set[str]) -> str:
    ...
    if has_study:
        parts.append(
            ...
            "the submission files named by each task contract plus its raw "
            "artifacts. Every study uses `manifest.json` and "
            "`evidence_report.json`; grounded protocols also use a structured "
            "study index, and v2 adds a prospective analysis intent. Legacy or "
            "evidence-bundle contracts may additionally require `metrics.json`, "
            "`provenance.json`, `methods.md`, or `decision_log.jsonl`. "
...
def export_benchmark_public_package(
    ...
    try:
        ...
        for relative, source_file in _V2_PUBLIC_VERIFIER_SOURCES.items():
            if not source_file.is_file():
                continue
            verifier_dest = staging / relative
            verifier_dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source_file, verifier_dest)
            tool_files.append(str(dest / relative))
            ...
            json.dumps(
                {
                    ...
                    # Do not disclose the canonical checkout path to a solver.
                    # The marker is only an overwrite-safety sentinel.
...
def export_benchmark_private_package(
    ...
    and top-level private references. Grounded Study tasks also carry a copy of
    the public ``prompt.md`` as scorer-owned, truth-blind judge context. It
    omits public submission contracts and checklists so it is not mistaken for
    an agent package.
    """
    source = Path(dataset_dir)
    if output_dir is None:
    ...
    try:
            ...
            if task.evaluation_protocol in {
                ...
                if not prompt_src.is_file():
                    return {
                        "success": False,
                        "errors": [
                            f"grounded Study task is missing prompt.md: {prompt_src}"
                        ],
                    }
                shutil.copy2(prompt_src, private_task_dir / "prompt.md")
                files_written.append(str(dest / "tasks" / task_id / "prompt.md"))
        ...
        readme.write_text(
            ...
            "`mdclaw score_benchmark_run`. For grounded Study tasks it also "
            "contains the same public `prompt.md`, used only as truth-blind "
            "judge context.\n\n"
    ...
    return {
        ...
        "included_public_judge_context": [
            "prompt.md for grounded_correct_v1 and grounded_correct_v2 tasks",
        ],
    }
