...
def write_benchmark_schemas(
    ...
    schemas = {
        ...
        "confirmatory_plan.schema.json": ConfirmatoryPlanV2,
        "claim.schema.json": ClaimV2,
    }
...
def _public_package_readme(primary_scores: set[str]) -> str:
    ...
    Grounded-correct-v2 tasks use the plan -> runner -> claim lifecycle. Legacy
    study contracts list their own author-written outputs. A mixed dataset gets
    ...
    if has_study:
        parts.append(
            "For grounded-correct-v2 scientific-answer tasks, first write "
            "`confirmatory_plan.json` and exit so the benchmark runner can "
            "freeze the plan and execute its pending production nodes. On "
            "continuation, inspect the certified result and write `claim.json`. "
            "The runner generates `manifest.json`, `episode/episode.json`, and "
            "the episode artifacts; agents must not create or edit those files. "
            "Legacy study contracts list their own author-written outputs.\n"
        )
    parts.append(
...
def export_benchmark_public_package(
    ...
    try:
        ...
        for task_id in task_ids:
            ...
            has_grounded_v2 = (
                has_grounded_v2
                or task.evaluation_protocol == "grounded_correct_v2"
            )
        ...
        # This helper only packages MDPrepBench's raw OpenMM bundle. Do not
        # expose it in a study-only package, where the benchmark runner owns
        # final packaging.
        if "preparation" in primary_scores and _STANDALONE_PACKAGER_SOURCE.is_file():
        ...
        if has_grounded_v2:
            schema_names[:0] = [
                "confirmatory_plan.schema.json",
                "claim.schema.json",
            ]
        elif primary_scores - {"preparation"}:
...
def export_benchmark_private_package(
    ...
    and top-level private references. It omits public prompts, submission
    contracts, and checklists so it is not mistaken for an agent package.
    """
    source = Path(dataset_dir)
    if output_dir is None:
