...
def submission_lifecycle(task: Task) -> dict[str, Any]:
    ...
    if task.evaluation_protocol == "grounded_correct_v2":
        return {
            "work_dir_policy": (
                "Use work_dir for source selection, preparation, exploratory "
                "simulation, and pending MDClaw production nodes."
            ),
            "submission_dir_policy": (
                "Before confirmatory MD, write confirmatory_plan.json in the "
                "exact submission_dir and exit. The benchmark runner freezes "
                "that plan and executes the pending nodes. On continuation, "
                "write only claim.json. The runner generates manifest.json and "
                "the episode ledger; do not create or edit them."
            ),
            ...
            "preflight_policy": (
                "No agent-side public preflight is required. After claim.json "
                "is written, the runner rebuilds the final package from its "
                "frozen plan and episode, then runs public preflight."
            ),
            "exit_condition": (
                "First exit after a valid confirmatory_plan.json references "
                "pending production nodes. After runner continuation, inspect "
                "the certified result, write claim.json, and exit."
            ),
            "background_policy": (
                "Do not run confirmatory production nodes yourself or leave "
                "unmanaged background work. The benchmark runner owns "
                "confirmatory execution."
            ),
            "agent_neutrality": (
                "Structure sources, preparation, exploration, and sampling "
                "allocation remain agent-selected. The task-owned observable "
                "and controls are replayed uniformly by the evaluator."
            ),
            "ownership": {
                "agent": [
                    "confirmatory_plan.json",
...
def submission_blueprint(task: Task) -> dict[str, Any]:
    ...
    if task.evaluation_protocol == "grounded_correct_v2":
        return {
            "confirmatory_plan_minimum": _confirmatory_plan_blueprint(task),
            "claim_minimum": _claim_blueprint(task),
            "runner_generated": [
                "manifest.json",
                "episode/episode.json",
                "episode/artifacts/",
...
def submission_checklist(task: Task) -> list[str]:
    ...
    if task.primary_score == PREPARATION_SCORE_AXIS:
        ...
        checks = [
            "confirmatory_plan.json parses and task_id matches this task",
            "every plan run has a unique run_id, a pending MDClaw prod node, "
            "and a job_dir under the assigned work_dir",
            "the plan includes reference and variant runs meeting the public "
            "minimum; do not execute them yourself",
            "write confirmatory_plan.json and exit for runner execution",
            "after continuation, derive claim.json only from the certified "
            "episode result",
            "a resolved claim uses one allowed outcome; an unresolved claim "
            "uses outcome=null",
            "do not create or edit manifest.json, episode/episode.json, or "
            "episode artifacts",
        ]
    else:
        checks = [
    ...
    if task.primary_score == PREPARATION_SCORE_AXIS:
        ...
        # The runner owns the execution ledger; v2 agents author no provenance
        # file in addition to confirmatory_plan.json and claim.json.
        pass
    elif stages:
        checks.append(
    ...
    if task.evaluation_protocol == "grounded_correct_v2":
        checks.extend(
            [
                "the evaluator replays the public task-owned observable and "
                "folded-state control from runner-owned episode artifacts",
...
def submission_checklist_markdown(task: Task, contract: dict[str, Any]) -> str:
    ...
    if lifecycle:
        ...
        if lifecycle.get("preflight_command_template"):
            lifecycle_lines.append(
                "- Public preflight: "
                f"`{lifecycle['preflight_command_template']}`"
            )
        elif lifecycle.get("preflight_policy"):
            lifecycle_lines.append(
                f"- Public preflight: {lifecycle['preflight_policy']}"
            )
...
def public_submission_contract(
    ...
    *,
    ...
    if task.evaluation_protocol == "grounded_correct_v2":
        contract["runner_episode_contract"] = {
            "agent_authored": [
                "confirmatory_plan.json",
                "claim.json",
            ],
            ...
            "public_preflight_scope": (
                "After the agent exits, the runner validates the finalized "
                "plan, claim, manifest, and episode envelope. Official scoring "
                "also verifies execution and replays the task-owned observable "
                "and controls."
            ),
            "primary_score": (
                "valid_execution AND claim_supported AND truth_agreement"
            ),
        }
    if task.evaluation_protocol == "grounded_correct_v2":
        ...
        contract["confirmatory_plan_schema"] = (
            "../../schemas/confirmatory_plan.schema.json"
        )
        contract["claim_schema"] = "../../schemas/claim.schema.json"
...
def _agent_required_outputs(task: Task) -> list[str]:
    if task.evaluation_protocol == "grounded_correct_v2":
        return ["confirmatory_plan.json", "claim.json"]
    if task.primary_score != PREPARATION_SCORE_AXIS:
...
def _manifest_output_blueprint(task: Task) -> dict[str, Any]:
    ...
    fixed_outputs = {
        ...
        "claim.json": ("claim", "claim.json"),
        "episode/episode.json": ("episode", "episode/episode.json"),
...
def manifest_list_output_requirements(task: Task) -> dict[str, int]:
    ...
    if task.evaluation_protocol == "grounded_correct_v2":
        # Runner-owned episode artifacts are the only source of truth for
        # confirmatory runs.
...
def _confirmatory_plan_blueprint(task: Task) -> dict[str, Any]:
    return {
        "schema_version": "1.0",
        "task_id": task.task_id,
        "runs": [
            {
                ...
                "job_dir": "<relative job directory under work_dir>",
                "node_id": "<pending MDClaw prod node ID>",
...
def _claim_blueprint(task: Task) -> dict[str, Any]:
    outcomes = (
