...
GUARDRAIL_CODES: dict[str, str] = {
    ...
    "residue_range_not_delivered": "Inspect source identity and coverage; build only confirmed missing residues, never fill author-number gaps.",
    "ligand_component_invalid": "Check the complete source selection, external covalent links and declared isomeric SMILES; do not manually rewrite the source to bypass validation.",
    ...
    "distance_steering_invalid": "Supply distance restraints or a custom script and positive steering times of at least one timestep.",
    "distance_steering_restart_mismatch": "Use matching XML, steering.json and (for custom steering) steering_initial.npz with the original bias and schedule.",
    "distance_steering_incomplete": "Resume the original steering schedule before starting fixed-center umbrella sampling.",
    ...
    "production_bias_conflict": "Choose one bias route: distance restraints, a custom script, or PLUMED.",
    "plumed_dependency_missing": "Build PLUMED and its plugin against the active OpenMM with container/scripts/build-plumed.sh.",
    "plumed_input_invalid": "Use the supported history-free PLUMED subset and consistent units, schedule and PRINT stride.",
    "plumed_restart_mismatch": "Keep matching XML state, plumed.dat and plumed.json with the original timestep and topology.",
    "plumed_steering_incomplete": "Resume the original steering_time_ns before creating a fixed umbrella continuation.",
    "plumed_output_invalid": "Inspect plumed.log and COLVAR; require finite CVs on the declared report grid.",
