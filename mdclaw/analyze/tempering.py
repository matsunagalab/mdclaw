...
def _plot_delta_f(path, times, pooled, per_run, labels, state_a, state_b, dF, checks, verdict,
    ...
    """dF(t) in the style of analyze_metadynamics: one panel, the second half
    shaded (green converged, red otherwise), the final value dashed; thin
    lines are the independent runs alone, black is all runs pooled."""
    try:
        import matplotlib
    ...
    ax.axvspan(times[-1] / 2.0, times[-1], color="tab:green" if verdict == "converged" else "tab:red", alpha=0.08)
    if n_runs >= 2:
        for r in range(n_runs):
            ax.plot(times, per_run[r], "-", lw=1.0, color=_RUN_COLORS[r] if r < len(_RUN_COLORS) else "gray",
                    label=labels[r])
    ax.plot(times, pooled, "k-", label="pooled" if n_runs >= 2 else None)
    if np.isfinite(dF):
        ax.axhline(dF, color="gray", lw=0.6, ls="--")
    ...
    if shown.size:
        pad = max(1.0, 0.1 * float(shown.max() - shown.min()))
        ax.set_ylim(float(shown.min()) - pad, float(shown.max()) + pad)
    ax.set_xlabel("simulation time per run / ns")
    ax.set_ylabel("dF(A - B) / kJ mol$^{-1}$")
    spread = checks["run_spread"]
    runs_txt = "" if spread is None else f", run spread {spread:.1f}"
    ax.set_title(f"RMSD A [{state_a[0]:g}, {state_a[1]:g}] vs B [{state_b[0]:g}, {state_b[1]:g}] nm "
                 f"at {checks['t_ref']:.0f} K\n"
                 f"dF = {dF:.1f} kJ/mol, second-half drift {checks['drift']:.1f}{runs_txt} ({verdict})",
                 fontsize=9)
    ax.grid(alpha=0.3)
    if n_runs >= 2:
