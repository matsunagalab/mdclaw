...
の formal charge を合算して `--charge_pdb_delta` に足す。標準 bare ion の価数は
`mdclaw/metal/_base.py` の `METAL_CHARGES` や `mdclaw/solvation/pdb_identity.py`
の formal-charge 判定を流用する。prep 出力の `component_disposition` / retained ion
情報から数える手もある。
...
- 金属価数: `mdclaw/metal/_base.py` の `METAL_CHARGES` と
  `mdclaw/solvation/pdb_identity.py` の formal-charge 判定
