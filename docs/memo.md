...
CLI 側の対策（同日、commit 予定）: `mdclaw/slurm/config.py` に `resolve_container_runtime()` を追加し、`submit_job` / `submit_array_job` / `submit_mps_job` が提出前にランタイム（`singularity` → `apptainer`、または `configure_container --runtime` の指定）を `MDCLAW_SLURM_PATH`（なければ PATH）上で絶対パスに解決して sbatch スクリプトに書く。イメージ内（`SINGULARITY_CONTAINER` あり）で解決できなければ `container_runtime_not_found` で拒否、イメージ外なら警告のみ（apptainer のないログインノードから投げる構成を壊さない）。スキル `hpc-run/SKILL.md` と `sif-slurm.md` に明記。ユーザーの指摘で `run_sst2` に溶媒混入ガード（`sst2_solute_includes_solvent`: 水・イオン・仮想サイトが solute に入っていれば拒否、mdtraj DSL の `resid` は全体番号なので起きやすい）も追加。今回の index ファイルは protein 鎖 0 のみで混入なし。
