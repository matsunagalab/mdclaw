...
Rerun spec: `runs/prep/experiment-kimi-k3-cli-failed-rerun.json` (45 tasks that failed at least once in a CLI condition x cli_skill_sif + cli_sif x 1 replicate = 90 attempts, 1800 s for membrane/antibody/nucleic, digest to be set to the accepted image); a confirmation run, v2 stays the baseline.
