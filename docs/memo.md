...
- **データセット（全 ITC、中性、各リガンド自身の holo 結晶構造）**: L99A 11 件は Morton, Baase & Matthews 1995（302 K、pH 5.5）: benzene 181L −5.19、toluene 4W53 −5.52、ethylbenzene 1NHB −5.76、n-propylbenzene 4W55 −6.55、n-butylbenzene 186L −6.70、isobutylbenzene 184L −6.51、p-xylene 187L −4.67、o-xylene 188L −4.60、benzofuran 182L −5.46、indene 183L −5.13、indole 185L −4.89。L99A の Mobley 2007 前向きセット 3 件（283 K）: 1,2-dichlorobenzene 2OTY −6.37、N-methylaniline 2OTZ −4.70、1-methylpyrrole 2OU0 −4.44（表面の 2 個目 MR3 304 は除外）。L99A/M102Q 11 件（283 K、pH 6.8）: Wei 2002 の phenol 1LI2 −5.24、3-chlorophenol 1LI3 −5.51、Boyce 2009 の catechol 1XEP −4.16、2-ethoxyphenol 3HU8 −4.02、2-propylphenol 3HTB −5.33、5-chloro-2-methylphenol 3HT8 −5.04、2-ethylphenol 3HT7 −4.57、2-methylphenol 3HT6 −4.44、N-phenylglycinonitrile 2RBN −5.52、2-nitrothiophene 2RBO −4.85、thieno[3,2-b]thiophene 3HUQ −4.67（kcal/mol）。値は benchmarksets（MobleyLab、LiveCoMS 2017）の tex、Boyce 2009 の Table 1、Mobley 2007 の Table 1/4 で照合済み。**Boyce 2009 Table 1 は 2RBN/2RBO を取り違えている**（RCSB では 2RBN = 264 (phenylamino)acetonitrile）。helix F が大きく動く benzyl acetate・tetrahydroindole、結合量が下限値しかない化合物は除外。実験値の幅は −4.02〜−6.70（2.7 kcal/mol）と狭いので R は出にくい。benchmarksets の注意どおり、propyl/butylbenzene は helix F の中間・開状態を取る。
- **σ（`--ligand-symmetry-number`）**: 回転子（メチル、アルキル、OH）を自由とみなした剛体骨格の回転群の位数で、benzene 12、p-xylene 4、C2 骨格（toluene、アルキルベンゼン、o-xylene、1,2-DCB、N-methylaniline、1-methylpyrrole、phenol、catechol、N-phenylglycinonitrile、thienothiophene）2、それ以外 1。自分の点群判断であり文献値ではない（Baumann 2021 は toluene に σ = 2 を使用）。σ 補正なしの値も併記する（`plot_exp_vs_calc.py --no-symmetry`）。
- **条件**: 各セットはそれぞれの実験温度で計算する（302 K / 283 K）。ff19SB / OPC / GAFF2(AM1-BCC)、複合体 12 Å・溶媒 15 Å の立方箱、0.15 M NaCl、HMR 4 fs。蛋白質の propka は実験 pH（Mobley 2007 の 3 件は pH が未確認なので 7.0）。リガンドは与えた中性 SMILES のまま（`--no-protonate-ligands`）。HED・BME・PO4・EPE・Cl⁻ は保持しない。
- **準備結果**: 25 件すべてが prep → solv → `build_decoupled_system` を両 leg 完了し、端点検証に合格した。複合体は 53,605〜58,685 粒子（|ΔE| coupled 0.51〜0.72 kJ/mol、decoupled < 0.05）、溶媒は 6,332〜8,052 粒子。ログインノードで 1 件あたり約 3 分。
- **見つけた問題 1（ツール）: 結晶水が残らない**。`--include-types ... water` を指定しても、`--keep-crystal-waters` がなければ INFO ログだけで捨てられる（`split.py:1080`）。フラグを付けても `split` は `water_1.pdb` を書くのに、`prepare_complex` の merge 入力（`prepare_complex.py:2933` 付近）に水のファイルが入らないので、結局 0 個になる。結果にも警告にも出ない。今回は結晶水なしで統一した。部位の水がリガンドに接しているのは 1LI2（phenol と水素結合する HOH 401）だけで、L99A の空洞は乾いている。**同日対応（ユーザー判断: 結晶水は当面対応外として一本化）**: `keep_crystal_waters` を `split_molecules` / `prepare_complex` から削除（CLI では `--keep-crystal-waters` が unrecognized arguments になる）。`include_types` に `water` があれば `split_molecules` が `crystal_waters_unsupported` で拒否し、`prepare_complex` はノードを pending のまま返す（引数を直して同じノードを再実行できる）。水残基は常に除去。テスト 2 本（`tests/test_prep_prechecks.py`、旧挙動では fail）、ガードレールコード 480 件。181L を実 CLI で確認: 拒否 → pending → `water` を外して同じノードが completed。
- **見つけた問題 2: `build_decoupled_system` のセグフォ**。ログインノードで 3 本並列に走らせると、ほぼ全件がトレースなしで落ちた（端点検証を CUDA で行う段）。単独実行でも 1 回落ちた（試行 4 回中）。ロックで直列化した再開パス（19 件 × 2 leg）では 0 件。落ちたノードは `running` のまま残るが、同じノードで再実行すれば完了する。`prep_ligand2.sh` はロックで直列化している。原因は未調査。
- **`run_fep` / `add_boresch_restraint` の温度は親から引き継がれず、既定が 300 K**（production は 87050da から引き継ぐ）。今回は全段で `--temperature-kelvin` を明示している。
- **GPU パイロット（ユーザー承認、Slurm 140942–140947、140990）**: benzene・n-butylbenzene・3-chlorophenol の各 1 レプリカ、窓あたり 0.2 ns 平衡化 + 2 ns、MPS 同居。min → eq1（拘束つき）→ eq2（拘束なし NPT、複合体 2 ns / 溶媒 1 ns）→ `add_boresch_restraint` → `run_fep`。経過時間は eq1 7 分、eq2 7 分、Boresch 1 分、溶媒 FEP 33 分（9 本同居）、複合体 FEP 78 分（8 本同居）で、計約 2.1 GPU-h。
  - **スループット（GB200）**: 複合体（約 5.5 万粒子）は 8 本同居で 1 窓あたり 240–253 ns/day、合計約 1,950 ns/day/GPU。溶媒（6–8 千粒子）は 9 本同居で 1 窓あたり 580–630 ns/day、合計約 5,400 ns/day/GPU。本計算（25 リガンド × 3 レプリカ × 窓あたり 5 ns）はリガンド・レプリカあたり約 2 GPU-h、計約 150 GPU-h の見積もり。
  - **結果**: n-butylbenzene は ΔG_bind = −6.89 ± 0.17 kcal/mol（σ = 2。σ なしなら −6.48、実験 −6.70）。complex leg 58.11 ± 0.44 kJ/mol（restrain 3.97 / decharge 5.29 / sterics 48.84、overlap 最小 0.068）、solvent leg −0.14 ± 0.56（overlap 最小 0.13）、拘束の解析項 +31.15、Boresch 原子は Ile78 N/C/CA ↔ 環の C1/C6/C5。3-chlorophenol（M102Q、283 K）は −7.20 ± 0.16（σ = 1、実験 −5.51、1.7 kcal/mol 結合しすぎ）。complex leg 166.77 ± 0.39（restrain 2.89 / decharge 126.62 / sterics 37.26、overlap 最小 0.118）、solvent leg 108.69 ± 0.54、解析項 +27.97、Boresch 原子は Val149。誤差は MBAR の統計誤差だけで、レプリカ 1 本・窓あたり 2 ns。
  - **benzene は `abfe_restraint_unstable`**: 距離の揺らぎは 0.029 nm で空洞に留まったまま、角度の揺らぎが 71.7°（上限 25°）。面内回転を 200 ps の平均で基準値にしようとしたのが原因で、ツールの案内「平衡化を延ばせ」は誤り。この MPS ジョブが FAILED 扱いになり、`afterok` 依存の複合体 FEP（140946）は自動キャンセルされた。`update_workflow_state --clear-slurm-metadata` で 8 ノードの記録を外し、2 系分を 140990 として投入し直した。
  - **拘束方式の調査（ソースとドキュメントで確認）**: OpenFE・BioSimSpace・BAT.py・Ries 2024・Aldeghi 2016・Mobley 2006/2007 はいずれも Boresch。重心拘束は YANK の T4L の例（受容体全体の重心に最も近い 1 原子が基準）と AToM。どの実装も受容体原子を基準にする。Mobley 2006 では距離だけの拘束でフェノールが 4–4.5 kcal/mol 結合しすぎ、サンプリングを延ばすほど悪化した。同じく Mobley 2006 は基準値にヒストグラムの最頻値を使い、σ 補正は「等価な向きが一度もサンプリングされない場合だけ」。Mobley 2007 はベンゼンを 1 つの向きに限定して σ で補正している。→ Boresch のまま、基準値を最頻値に変え、σ を λ = 0 の窓からツールが判定する方向で提案中（ユーザーの判断待ち）。
  - **ベンゼンの運動の実測（9/27、`diag_benzene_rotation.py`、eq_002 から 200 ps、ログインノードの GPU）**: 環の重心–Val94 CA 距離の std 0.024 nm、環の法線の向きの std 7.5° — 位置も面の向きも固定。面内回転はラベル付き C1 が 200 フレームで 6 つの等価な向きすべてを訪れ、乗り換え 103 回（ps 時間スケールで自由）。裏返し（受容体側から見た C1→C2→C3 の向き）は 200 ps で 0 回。ツールが見るラベル付き Boresch 座標では最良候補（Val94 主鎖 ↔ C2/C1/C6）でも φ_B の std が 86°、他は ≤ 16°、r は 0.04 nm。既定の拘束 λ（0.02 / 0.08 / 0.2 / 0.5 / 1）の実効幅は角度 70 / 35 / 22 / 14 / 10°、距離 0.17 / 0.09 / 0.055 / 0.035 / 0.024 nm。**結論**: 拒否は「面内回転の平均を基準にした」ことだけが原因。回転が λ_r = 0 で完全にサンプリングされるので、拘束相の FEP が 6 通りの閉じ込めコスト（kT ln 6 ≈ 1.1 kcal/mol）を自動的に含み、面内回転の σ 補正は不要（入れると二重計上）。裏返し分（kT ln 2 = 0.42 kcal/mol）だけがサンプリングされない残りで、σ 自動判定を作らずに「補正しない近似」として明記するのが最も単純。
...
The shared path `/data1/rkp00079/mdclaw-rikyu-arm64-cuda130-cufft121-fusefix-6f171d2f0fa5.sif` now links to `…-tempinherit-20e916d447b9.sif` (switched 15:35 JST; sha256 `51bb13ca5809ae704d502c5c6c57faea143d53f627040961420207ed3ce12771`, 7,439,286,272 bytes, labels `org.mdclaw.source.commit 87050da…`, `runtime.bundle_id 20e916d447b9`, SST2 5590f4f carried). Rollback link `…fusefix-6f171d2f0fa5.pre-tempinherit-20260926.sif` → the modgeom image `…-modgeom-28f5e4a4f461.sif` (fa683b6a…, main b6b7721).

Built with the usual recipe (`.validation/tempinherit-20260926/`: `build-candidate.sh`, `acceptance.sbatch`, `activate-tempinherit.py`, `update-rikyu-md.py`) as a source overlay of the committed tree on the modgeom image: 67 packaged files replaced or added (the temperature inheritance of 87050da plus FEP/ABFE, rounds/WE, metadynamics and Slurm recovery merged since b6b7721), all 224 packaged files equal main 87050da; no dependency, environment or container change, and every mdclaw submodule of 87050da imports in the base image. Accepted on GB200 nodes: job 140590 (c154) baked-package CLI 98 tools, CUDA, the 343k-atom TIP3P 3WD5 cell built through the Pablo fast path in 60 s with rigid water and 500 clean iterations; job 140723 (c074) eq at 310 K then `run_production` without `--temperature-kelvin` on CUDA ran at 310 K (integrator XML and signature), `temperature_kelvin_source=inherited` from eq_001; job 140749 (c154) fast suite against the baked package 2,597 passed / 2 skipped (three tests deselected that fail identically on main before the change). Two harness slips in my acceptance script, not image faults: 140590's CUDA check could not import `tests.pipeline_helpers` (another `tests` package in the image shadows the checkout's; now loaded by file path), and its suite summary was pushed out of the captured tail by `dcdplugin` lines (now filtered); both reran as their own jobs above.

Before the switch, the user's 18 pending t1r production jobs were checked: every one passes `--temperature-kelvin 300` explicitly and continues at matching settings, so the new image does not change what they compute.

The pi package checkout (`~/.pi/agent/git/github.com/matsunagalab/mdclaw`) was then updated with `pi update git:github.com/matsunagalab/mdclaw@main` from dd7a672 to 87050da (skills present, including md-fep, md-abfe and md-we), after the image, so agents never read "omit the production temperature" against an image that would default to 300 K. RIKYU.md updated in both copies. New MDDataBench specs must pin `sif_sha256` 51bb13ca….

## 2026-09-26 — Production takes its temperature from the node it restarts from; rounds pin one temperature per scheme

Why: MDDataBench glm-5.3-flash-3cond-full, `010_membrane_6kux` cli_skill_sif r3. The prompt asked 310 K, the agent ran `run_equilibration --temperature-kelvin 310` and then `run_production` without the flag (a Slurm chain, afterok), so production ran at the 300 K default and failed both temperature checks. The eq -> prod temperature change had been a hard error until 17afe45 (2026-09-10), which relaxed timestep, temperature and friction together to warnings because of a timestep case (011_membrane_6kuy: eq recovered at 2 fs after a NaN, the default 4 fs production was refused); no reason specific to temperature, and none from the enhanced-sampling work (run_sst2, run_metadynamics and run_fep build their own integrators and never pass that check), was recorded. This narrows only the default, not 17afe45: an explicit different temperature from an eq still runs with a warning.

Surveyed first (five lenses plus a critic, then an adversarial review with per-finding verification, 32 agents). Inheriting alone was not free of side effects: the Slurm preflight filled an omitted flag from the signature default and would have refused every declared-temperature prod command once the default became None; the value has to come from the node whose state is loaded (a continuation after an explicit 300 K production from a 310 K eq must stay at 300 K, so the eq-ancestor rule `pressure_bar` uses is wrong here) and be resolved before the declared-condition check; and rounds/WE schemes would mix temperatures (a 310 K start ran every segment at 300 K before; after naive inheritance, continued walkers of a running scheme would keep 300 K while recycled ones took the basis's temperature).

What changed:
- `mdclaw/node/inputs.py` `_resolve_restart_temperature(job_dir, restart_node_id)`: the restart node's `integrator_signature.temperature_kelvin` (what the restart check compares, so an inherited value never trips it), else `metadata.temperature_kelvin` (SST2 nodes); never a further ancestor, never state.xml. `resolve_node_inputs` for eq and prod returns it, so `explain_node` shows it.
- `run_production`: `temperature_kelvin: Optional[float] = None`. In node mode the restart node and the temperature are resolved before `validate_node_execution_context`; omitted -> inherited from the restart node, else 300 K with a warning saying why (no recorded temperature, or an explicit restart file matching no ancestor); standalone 300 K. `temperature_kelvin_source` (`inherited` / `explicit` / `default`) and `temperature_kelvin_inherited_from` in the result and node metadata, and a warning naming the source node. An explicit temperature or timestep different from a prod parent's signature is now refused after the node-context check and before anything runs (`production_restart_integrator_mismatch`); it used to fail the node after it began, with no code, and `run_rounds` retried it into `rounds_replica_unstable`. A node run directly stays pending; a submitted (queued) one is recorded failed and must be replaced, which the envelope says (the registered text no longer claims either).
- `run_metadynamics`: the same inheritance (a new walker takes the eq's temperature, a continuation its parent walker's), resolved before kT-dependent parameters and the manifest.
- `run_equilibration` does not inherit (after md-we's deliberate 500 K eq a forgotten flag would carry the hot stage forward) but refuses an omitted temperature when it restarts from an eq that ran at a temperature other than 300 K (`eq_restart_temperature_unstated`); min -> eq and 300 K chains are unchanged.
- `mdclaw/slurm/preflight.py`: an omitted `--temperature-kelvin` is not compared before submission (a declared one is deferred to the run); an explicit one is compared.
- Rounds/WE: `setup_rounds` pins `segment_temperature_kelvin` from the start and explicit basis nodes (disagreement: `rounds_start_temperature_mismatch`), reports `segments_run_at_kelvin`, and refuses a `stage_args` temperature/timestep a prod start or basis node could not be continued with; the driver passes the temperature explicitly to every segment (local, MPS per-task and batch commands), 300 K for schemes recorded before this so a running scheme keeps its temperature; `we_resample` checks basis nodes; the round ledger records `segment_temperatures_kelvin`; `analyze_we` takes kT from the segments and refuses mixed temperatures whether or not `--temperature-kelvin` is given.
- Skills: md-production SKILL.md Prerequisites is the one home of the rule (the temperature is given once, to `run_equilibration`; production commands omit the flag); explicit-water, implicit-water, restart, custom-force, metadynamics, rounds, sst2, md-we, md-equilibration (every eq stage states its temperature), common/solvent-regimes and the hpc-run chain and extension pages follow it; tool-reference documents the tools and codes.
- Three guardrail codes registered: `production_restart_integrator_mismatch`, `eq_restart_temperature_unstated`, `rounds_start_temperature_mismatch`.

Tests: new `tests/test_production_temperature_inheritance.py` (12, real eq/prod nodes on the two-carbon periodic triple, Reference platform: eq 310 -> prod 310 inherited; continuation keeps an explicit 320; explicit mismatch on continuation refused with the node pending, then the same node runs without the flag; declared conditions compared with the inherited value; legacy eq and standalone at 300; explicit 300 wins; explain_node; preflight deferral; terminal node gets node_terminal), `tests/test_eq_restart_temperature.py`, `tests/test_rounds_temperature.py` (8, including pre-pin schemes, MPS commands, run_sst2, prod start nodes, we_resample basis, analyze_we), metadynamics additions. Fast suite: 2597 passed, 2 skipped; the three failures (`test_envelope::test_alchemical_routing_is_decided_per_branch`, `test_pdb_export_resname_guard::test_pdb_writefile_inventory_is_pinned`, `test_restraint_selection::test_direct_distance_restraint_reporter_matches_custom_cv_reference`) fail identically on a clean HEAD copy. ruff clean.

Rollout: the new skill text tells agents to omit `--temperature-kelvin` in production. The shared RIKYU SIF (`modgeom-28f5e4a4f461`, mdclaw b6b7721) and the pinned pi checkout (dd7a672) do not have the inheritance code; if the skills reached agents before the image, every non-300 K production would run at 300 K. Repin the pi checkout only together with a SIF rebuilt from this commit.

Left: `pressure_bar` still inherits from the nearest eq ancestor (a continuation after an explicit NVT production of an NPT eq switches back to NPT with a warning), `run_fep` and the SST2 reference rung still default to 300 K independently of the eq, and `explain_node` on an eq -> eq node shows the parent's temperature and `ready_to_run` although omitting the flag is refused.
