...
- **Scorer/CI dry runs** use synthetic submissions generated by
  `tests/test_benchmark/_fake_submissions.py`. They exercise the dataset
  schema, validation, scoring, aggregation, and regression checks without
  running MD. These runs are not leaderboard evidence.
...
`tests/test_benchmark/test_all_task_dryrun.py` uses synthetic submissions from
`tests/test_benchmark/_fake_submissions.py` to validate, score, and summarize
all nine tasks in both `honest` and `wrong` modes. It intentionally
