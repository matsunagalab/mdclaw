...
- Multiple `--residue-ranges` pieces from one polymer become separate components
  by default. Use `--join-range-pieces` only when the user explicitly wants a
  join; PDB `CONECT` records do not suppress standard peptide bonds.
