...
mdclaw create_node --job-dir "$JD" --node-type prod --continue-from <completed_prod_node_id> \
  --label extend_100ns --conditions '{"simulation_time_ns":100}'

mdclaw submit_job \
  --job-dir "$JD" --node-id <extension_prod_node_id> \
  --script "mdclaw --job-dir $JD --node-id <extension_prod_node_id> run_production \
