# P33_prep_physiological_nacl_ubiquitin: MD system preparation

You are evaluating an MD agent on `P33_prep_physiological_nacl_ubiquitin`.

Use this prompt as the task statement. Retrieve public sources as needed, and do not read `truth/` or `scorer/` if those directories exist.

Task: Physiological salt concentration: build an explicit-solvent ubiquitin system (PDB 1UBQ) at a physiological 0.15 M NaCl concentration while preserving net neutrality, using NaCl (not KCl) as the requested salt.
