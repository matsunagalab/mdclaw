...
For machine-readable scoring, record `preparation.assembly_id = "1"` and `preparation.assembly_chain_identity_map` in `metrics.json`. The identity map should cover generated output chains and include `source_pdb_id`, `assembly_id`, `source_auth_asym_id`, `source_label_asym_id` or `source_subchain_id`, `operator_id`, `output_chain_id`, and `naming_policy`. Also document the source PDB and multi-chain identity policy in `provenance.json` or `evidence_report.json`. The submitted structure should represent assembly 1 rather than the asymmetric unit alone. The expected assembly chain count refers to the polymer (protein/nucleic) chains of the biological assembly; bound cofactors or ligands may be assigned their own chain IDs and are not counted toward that total. Tag each map entry with `molecule_type`.
