...
class TestProtocol:
    ...
    def test_schedule_from_parameter_dicts(self):
        sched = [dict(STATE_A), {**STATE_A, "fep_elec_old": 0.5}, dict(STATE_B)]
        windows = windows_from_schedule(sched)
        assert windows[1]["lambda"] is None
        assert windows[1]["parameters"]["fep_elec_old"] == 0.5

    @pytest.mark.parametrize("bad", ["[0.2, 1]", "[0, 0.7]", "[0]", "[0, 1.5]"])
    def test_schedule_rejects_bad_endpoints(self, bad):
...
class TestAnalysis:
    ...
    def test_estimate_ddg(self, tmp_path):
        ...
        res = estimate_ddg(str(folded), str(unfolded))
        assert res["success"], res
        ...
        assert Path(res["ddg_file"]) == tmp_path / "ddg.json"
...
class TestDag:
    def test_fep_node_parents(self, tmp_path):
        ...
        # fep -> analyze allowed with a production_chain scope
...
class TestHybridVacuum:
    ...
    @pytest.mark.parametrize("old,new", [("LEU", "ALA"), ("GLY", "PRO")])
    def test_endpoints_reproduce_end_states(self, tmp_path, old, new):
        ...
        from mdclaw.fep.mapping import atom_records_from_topology, bonds_from_topology, find_residue_index
        ...
        mapping = map_mutation(ra, rb, find_residue_index(ra, "A", "2"),
