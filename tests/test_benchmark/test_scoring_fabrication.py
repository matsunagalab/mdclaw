...
def _write_haiku_v1_t06(submission_dir: Path):
    """Pin the previously observed Haiku v1 shape without keeping separate
    fixture files on disk."""
    submission_dir.mkdir(parents=True, exist_ok=True)
    manifest = {
        "limitations": [
            "Generated by create_benchmark_submission_template; fill "
            "task-specific metrics, evidence, and artifacts before scoring.",
        ],
        "outputs": {
            "checkpoints": [],
            "decision_log": None,
        ...
        "run_id": "20260511_mdclaw_fullrun",
        "schema_version": "1.0",
        "status": "partial",
        "task_id": "T06_answer_stability_t4l_l99a",
    }
    metrics = {
        "answer": {
            "status": "completed",
            "effect_direction": "destabilizing",
            "confidence": "moderate",
            "evidence": "ΔΔG ≈ +2.3 kcal/mol from structure",
    ...
    evidence = {
        ...
        "limitations": ["Framework submission for benchmark validation"],
        "effect": {"direction": "destabilizing", "confidence": "moderate"},
    }
    provenance = {
        "task_id": "T06_answer_stability_t4l_l99a",
        "generated_by": "MDClaw benchmark framework",
        "timestamp": "2026-05-11T16:00:00Z",
        "status": "completed",
    }
...
def test_warn_policy_pins_real_haiku_v1_t06_regression_fixture(tmp_path: Path):
    ...
    task.scoring.integrity_policy = "warn"
    submission_dir = tmp_path / "submission"
    _write_haiku_v1_t06(submission_dir)
