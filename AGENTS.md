...
- `build_openmm_system(pdb_file, forcefield_xml, additional_smiles, nonbonded_method, nonbonded_cutoff_nm, constraints, rigid_water, hmr=True, minimize, output_name, job_dir, node_id)` - **Research-mode escape hatch**: skips `forcefield_catalog` entirely and applies whatever `forcefield_xml` list the caller provides (e.g. `["GB99dms.xml"]`, third-party Amber conversions, custom ports). Uses Pablo for topology and `openmm.app.ForceField.createSystem(...)`; emits the same `system.xml` + `topology.pdb` + `state.xml` triple so eq/prod consume both builders identically. `hmr=True` bakes `hydrogenMass=4 amu` into the System and records `forcefield_provenance.method.hmr`. Node-mode contract mirrors `build_amber_system`: surfaces `forcefield_xml`/`nonbonded_method`/`nonbonded_cutoff_nm`/`constraints`/`rigid_water`/`hmr`/`output_name` as `actual_conditions` so a topo node declared with `conditions={"hmr": True}` (etc.) gates the build. Minimal guardrails (e.g. block GB99 names against OpenMM < 8.0); no FF×water matrix.
