...
mdclaw/                    # All Python code consolidated here
  ...
  amber_server.py           # Curated Amber→OpenMM system builder
                            # (openmmforcefields + Pablo, no tleap)
  openmm_system_server.py   # Research escape hatch: arbitrary OpenMM ForceField XML
  md_simulation_server.py   # OpenMM MD execution (consumes system.xml triple)
...
6. Update AGENTS.md server section with the new tool signature
...
5. Update AGENTS.md architecture diagram and server section
  ...
  nodes/
    ...
    topo_001/
      ...
      artifacts/
        ...
        system.topology.pdb        # OpenMM Topology with bonds + box vectors
        system.state.xml           # post-minimization positions/velocities/box
    eq_001/
      node.json
      node.lock
      artifacts/
...
If preserving ligands while narrowing chains, include the ligand chain(s) from
`inspect_molecules` too; `select_chains=["A"]` alone can intentionally produce
a protein-only prep when hetero ligands live on separate subchains.
...
# Level 3: Standard DNA/RNA topology DAGs (network + openmmforcefields)
pytest tests/test_pipeline_nucleic_dag.py -v

# Level 3: Modified nucleic acid DAG (requires MDCLAW_MODXNA_DIR;
# currently fail-fast under the openmmforcefields path until the
# parmed bridge ships — see modxna_openmm_xml_required code)
pytest tests/test_pipeline_modxna_dag.py -v
...
- `inspect_molecules(structure_file, job_dir, node_id)` - Analyze chains, nucleic acids, glycans, ligands, ions. Standard DNA/RNA polymers are classified as `nucleic` (not ligand) with `summary.nucleic_label_ids` and `summary.nucleic_subtypes`; common glycan residues / carbohydrate entities are classified as `glycan` with `summary.glycan_label_ids`; modified nucleotides are detected as unsupported for the standard NA path and reported under `summary.modified_nucleic_residues` with targetable chain/residue fields when available. With `job_dir`/`node_id`, writes `inspection.json` under the node and emits an `inspection_completed` event (read-only — node status unchanged). `summary.ptm_residues` lists any SEP/TPO/PTR sites present (chain ids are the **source** chain ids, before any `merge_structures` reassignment), and `notes.ptm_handling` carries the handoff message to `phosphorylate_residues`.
...
- `prepare_modified_nucleic(modifications, modxna_dir, job_dir, node_id)` - Generate modXNA parameters on a branched `prep` node after `prepare_complex`. Node mode only: input PDB auto-resolves from the nearest prep ancestor's `merged_pdb`, and source-coordinate targets resolve through ancestor `residue_mapping`; pass `coordinate_frame="merged"` to target merged PDB chain/residue IDs directly. Each modification needs `chain`, `resnum`, `source_resname`, and `backbone` / `sugar` / `base` fragment IDs unless a curated preset exists (currently `5CM -> DPO/DC2/M5C`). The tool runs modXNA once per unique fragment triplet, records generated `.lib` + `frcmod.modxna` as `modxna_params`, renames only resolved residues in `modified_nucleic.pdb`, and registers that file as downstream `merged_pdb`. Terminal 5′/3′ modifications remain blocked with positional diagnostics.
- `phosphorylate_residues(pdb_file, sites, sites_str, restore_from_detection, allow_partial, name, output_dir, job_dir, node_id)` - Apply phosphorylation (SER→SEP / THR→TPO / TYR→PTR) on a branched `prep` node after `prepare_complex`. Three input modes (mutually exclusive): `restore_from_detection=True` reads `metadata.detected_ptm_residues` from the nearest prep ancestor (re-introduces PTMs that the source PDB carried but PDBFixer stripped); `sites=[{"chain","resnum","target"}, ...]` for explicit sites; `sites_str="A:65:SEP,A:178:TPO"` for the same as a CLI string. Each site's current residue must be the standard counterpart of the target — mismatches return a structured error. Sites that cannot be located in the input PDB (typo, chain-id drift) are **fatal by default**; pass `allow_partial=True` to convert to a warning. The tool renames the residue and strips the hydroxyl H (`HG`/`HG1`/`HH`), keeping `OG`/`OG1`/`OH`; `build_amber_system` then routes the matching `phosaa19SB`/`phosaa14SB`/`phosaa10`/`phosfb18` XML into the `openmmforcefields.SystemGenerator` bundle so the phosphate atoms are rebuilt by the OpenMM ForceField (the legacy `leaprc.phosaa*` tleap source line is no longer involved). The phosphorylated PDB is registered as both `merged_pdb` and `phosphorylated_pdb` for downstream auto-resolve. DAG: source → prep (clean) → prep (phosphorylate) → solv → topo → eq → prod.
...
### amber_server.py
- `build_amber_system(pdb_file, ligand_params, modxna_params, metal_params, box_dimensions, forcefield, water_model, nucleic_forcefield, glycan_forcefield, is_membrane, hmr=True, implicit_solvent=None, job_dir, node_id)` - **Curated Amber→OpenMM system builder** (no tleap). Routes the prepared PDB through OpenFF Pablo for chemistry-aware topology loading, then builds the OpenMM `System` via `openmmforcefields.SystemGenerator` + `GAFFTemplateGenerator` against the XML bundle resolved from `forcefield_catalog`. Outputs the modern artifact triple `system.xml` + `topology.pdb` + `state.xml` (post-minimization positions/velocities/box) and stamps the topo node with `metadata.system_artifact_kind="openmm_system_xml"` and a `metadata.forcefield_provenance` dict (resolved OpenMM XML names, sha256 table, OpenMM / openmmforcefields / openff-toolkit versions, `method.hmr` / `method.hydrogen_mass_amu`, ligand Molecules). HMR is a build-time choice: the default `hmr=True` bakes `hydrogenMass=4 amu` into `system.xml` so the run-side default 4 fs timestep is loadable. In node mode, `pdb_file` auto-resolves from `solv` ancestor's `solvated_pdb` or nearest prep `merged_pdb`, and `modxna_params` / `glycan_metadata` / `glycan_linkages` auto-resolve from the nearest prep ancestor. The catalog selects DNA `OL15`/RNA `OL3` XMLs when standard nucleic residues are present (`nucleic_forcefield="auto"`), and `GLYCAM_06j-1.xml` for glycan residues (`glycan_forcefield="auto"`); `_prepare_glycam_pdb_with_cpptraj` still runs upstream so GLYCAM-named glycans match the converted XML templates, and protein-glycan / SS bonds are added to the OpenMM Topology before `create_system`. PTM detection is preserved: `detect_ptm_sites` scans for SEP/TPO/PTR and the matching `phosaa19SB`/`phosaa14SB`/`phosaa10`/`phosfb18` XML is added to the SystemGenerator bundle (never loaded as `leaprc`). A protein FF with no paired phosaa XML when PTMs are present returns `phospho_forcefield_unsupported`. Implicit solvent: pass `implicit_solvent="HCT" / "OBC1" / "OBC2" / "GBn" / "GBn2"` (case-insensitive; `igb1`–`igb8` and `gbneck2` aliases also accepted). The matching `implicit/*.xml` is added to the SystemGenerator bundle so the saved `system.xml` carries a `CustomGBForce` / `GBSAOBCForce`; the run-side shim verifies that force is present before honoring `--implicit-solvent`. Combining `implicit_solvent` with `box_dimensions` returns `implicit_solvent_explicit_box_conflict`; an unknown model name returns `implicit_solvent_model_unsupported`; a built System missing the expected GB force returns `implicit_solvent_force_missing`. `forcefield="ff14SB"` is auto-substituted to the GBneck2-tuned `ff14SBonlysc` variant when `implicit_solvent` is set (with a warning); `ff19SB` + `implicit_solvent` is allowed but warns because ff19SB is OPC-tuned. For non-shipped GB models (e.g. the Greener group's `GB99dms.xml`), route through `build_openmm_system`. Out of scope (still): under the openmmforcefields path, `metal_params` and `modxna_params` cannot yet be converted to OpenMM ForceField XML, so they fail-fast with structured codes `metal_openmm_xml_required` / `modxna_openmm_xml_required` (research users supply pre-built XML through `extra_xml`). Standard ligands flow through GAFFTemplateGenerator unchanged. `build_amber_system` reflects the curated/recommended path; the parallel `build_openmm_system` is the research escape hatch that accepts arbitrary OpenMM XML.

### openmm_system_server.py
- `build_openmm_system(pdb_file, forcefield_xml, additional_smiles, nonbonded_method, nonbonded_cutoff_nm, constraints, rigid_water, hmr=True, implicit_solvent=None, minimize, output_name, job_dir, node_id)` - **Research-mode escape hatch**: skips `forcefield_catalog` entirely and applies whatever `forcefield_xml` list the caller provides (third-party Amber conversions, custom ports, e.g. `["GB99dms.xml"]`). Uses Pablo for topology and `openmm.app.ForceField.createSystem(...)`; emits the same `system.xml` + `topology.pdb` + `state.xml` triple so eq/prod consume both builders identically. `hmr=True` bakes `hydrogenMass=4 amu` into the System and records `forcefield_provenance.method.hmr`. Node-mode contract mirrors `build_amber_system`: surfaces `forcefield_xml`/`nonbonded_method`/`nonbonded_cutoff_nm`/`constraints`/`rigid_water`/`hmr`/`implicit_solvent`/`output_name` as `actual_conditions` so a topo node declared with `conditions={"hmr": True}` (etc.) gates the build. Minimal guardrails (e.g. block GB99 names against OpenMM < 8.0); no FF×water matrix. Implicit solvent has two tiers: (a) **shipped GB XML** — pass `forcefield_xml=[..., "implicit/<model>.xml"]` *with* `implicit_solvent="<MODEL>"` (HCT / OBC1 / OBC2 / GBn / GBn2; `gbneck2` / `igb1`–`igb8` aliases accepted) so the canonical name lands on `metadata.implicit_solvent` and the run-side topology guard matches build-time vs runtime choices. The XML is checked against `forcefield_catalog.IMPLICIT_SOLVENT_XML`; a missing entry returns `implicit_solvent_xml_missing`, multiple shipped GB XMLs without an explicit declaration returns `implicit_solvent_xml_ambiguous`, and a built System with no `GBSAOBCForce` / `CustomGBForce` returns `implicit_solvent_force_missing`. (b) **External GB XML** (e.g. `GB99dms.xml`) — loadable as arbitrary OpenMM XML, but mdclaw cannot canonicalize a non-catalog GB XML, so `metadata.implicit_solvent` stays `None` and the run-side topology guard cannot validate the build/runtime match. Advanced users must own XML correctness, GB-force presence, and consistency between build and run themselves.

### md_simulation_server.py
- `inspect_openmm_platforms(atom_count, solvent_type)` - Lightweight preflight that enumerates available OpenMM platforms and classifies local explicit-water feasibility from atom count. Use after solvation and before local eq/prod; large explicit systems without CUDA/OpenCL should be routed to HPC or deliberately shortened smoke tests rather than launched silently on CPU.
- `run_equilibration(system_xml_file, topology_pdb_file, state_xml_file, temperature_kelvin, pressure_bar, nvt_time_ns, npt_time_ns, nvt_steps, npt_steps, restraint_atoms, restraint_force_constant, hmr=True, ..., job_dir, node_id)` - Standard staged minimization + low-temperature NVT warmup + NVT, followed by NPT when applicable. Prefer `nvt_time_ns` / `npt_time_ns` (CLI: `--nvt-time-ns` / `--npt-time-ns`) for user-facing duration requests so agents do not convert ns/ps to steps themselves; `nvt_steps` / `npt_steps` remain low-level explicit-step overrides and are mutually exclusive with the matching time flag. In node mode the XML triple (`system_xml_file`/`topology_pdb_file`/`state_xml_file`) is auto-resolved from the topo ancestor via `_resolve_topology_files` (atomic — all three artifacts must come from the *same* topo node). Each NVT / NPT / clean-handoff stage deserializes a fresh `openmm.System` from `system.xml` via `_deserialize_xml_system` (so restraint / barostat additions do not leak between stages) and runs `_validate_xml_system_contract` against it: a runtime `hmr=True` against a non-HMR build raises `_ModernSystemContractError(modern_system_hmr_mismatch)`, and `--implicit-solvent` against a System without a GB force raises `modern_system_implicit_solvent_unsupported`. The build vs runtime implicit-model match is enforced upstream by the topology guard (`implicit_solvent_topology_mismatch`). Begin_node-after early returns flow through `_fail_node_if_running` so the node never lingers in `running` after a failure. Missing XML inputs surface as `missing_xml_topology_inputs`; the run side never reads parm7/rst7.
- `run_production(system_xml_file, topology_pdb_file, state_xml_file, simulation_time_ns, ..., platform, device_index, restart_from, hmr=True, random_seed, output_frequency_ps, job_dir, node_id)` - Production MD (HMR + 4fs default). In node mode the XML triple resolves from the `topo` ancestor; `restart_from` walks the DAG upward and **prefers the ancestor's `state` artifact (XML, cross-node portable); the `.chk` artifact is kept for same-GPU bit-exact replay (committor / sensitivity analyses), not as a topology fallback**. The XML triple itself is the only topology contract — there is no parm7/rst7 fallback. Resolution picks the nearest `prod` ancestor with a `state`/`checkpoint` artifact (extension case), falling through to the `eq` ancestor if no prod ancestor has one (fresh production). A *completed* prod / eq ancestor that registered neither artifact (broken-DAG signal — e.g. a tool wrote a trajectory but forgot to save state) surfaces as `restart_from_error` and the run node is failed; the resolver refuses to skip it and silently roll back to an older ancestor. When the user passes an explicit `--restart-from <path>` instead, the path is matched back to a DAG ancestor's `state`/`checkpoint` by absolute-path equality so `simulation.currentStep` (read from `metadata.final_step`) tracks the *same* node whose artifact is loaded — an external file that does not match any ancestor leaves `currentStep` to whatever the loader sets. XML state restart downgrades `ensemble` / `pressure_bar` mismatches to warnings (`_load_state_into_simulation` drops barostat parameters); binary `.chk` keeps every signature key as a hard error. The reproducibility signature is keyed on `system_xml_sha256` / `topology_pdb_sha256` / `state_xml_sha256` so eq → prod handoff detects ancestor edits between runs. On load, `.xml` triggers `Simulation.loadState` and `simulation.currentStep` is restored from `node.json.metadata.final_step` of that ancestor (saveState does not persist the step counter); `.chk` triggers `Simulation.loadCheckpoint` which carries the step counter itself. Both formats are saved at end-of-run and periodically (via two `CheckpointReporter` instances, one with `writeState=True`); no code path reads the `.chk` when a `.xml` is also present. `simulation_time_ns` is the time to run **in this call** — it is added on top of the restart state's `currentStep`, so `eq→prod` keeps its "full production length" meaning (the eq state is written with `final_step=0` by design) while `prod→prod` cleanly extends by the requested duration. The node records `start_step` / `start_time_ns` / `final_step` / `output_frequency_ps` so analysis and Methods reports can place each segment on the right timeline and recover reporter cadence, and DCDs are never appended across nodes. Begin_node-after early returns are wired through `_fail_node_if_running`.
...
- `inspect_job(job_dir)` - Read-only job summary for weak-agent re-entry: status counts, leaf nodes, pending/running/failed lists, open needs, claims, progress warnings, and the progress index.
- `explain_node(job_dir, node_id, expected_node_type, actual_conditions)` - Read-only node explanation with parent/dependency statuses, artifact keys, metadata errors, execution-context validation, and auto-resolved inputs.
...
- `register_visual_review(image_path, output_dir, output_name, reviewer_type, severity, recommendation, summary, checks, findings, limitations, source_node_id, source_artifact_key, reviewer_model, review_prompt, job_dir, node_id)` - Register the outcome of a human or multimodal-LLM visual QA pass as `visual_review_json`. This is a coarse accident check only (missing components, obvious separation, impossible overlap, broken membrane placement), not scientific validation. `reviewer_type="not_available"` records that image review could not be performed; `severity="high"` / `recommendation="user_confirm"` asks the agent to pause for user confirmation but does not fail the node.
...
export MDCLAW_AMBER_TIMEOUT=3600   # build_amber_system wall-time budget for the
                                   # openmmforcefields SystemGenerator + initial
                                   # LocalEnergyMinimizer pass; raise for very
                                   # large fusions / glycoproteins.
...
`build_amber_system` adds the matching phosaa OpenMM ForceField XML to
its `SystemGenerator` bundle automatically — `amber/phosaa19SB.xml`
(ff19SB) / `amber/phosaa14SB.xml` (ff14SB) / `amber/phosaa10.xml` (ff03,
ff99SB family legacy) / `amber/phosfb18.xml` (fb15) — so the residue
template comes from the openmmforcefields conversion, not the legacy
`leaprc.phosaa*` tleap path. Caveat: the `protein.ff14SB.xml` ↔
`phosaa14SB.xml` atom-type asymmetry in openmmforcefields means
ff14SB + phosphorylation currently fails with `KeyError: 'N'` until the
upstream packaging is fixed; ff19SB + phosaa19SB is the supported
combination.
...
- **Crystallographic phosphate-coordinate preservation**. v1 destroys
  and rebuilds phosphate atoms via the openmmforcefields phosaa
  templates; OK for MD setup (minimization absorbs it) but a regression
  for users who care about the original phosphate orientation. A future
  "preserve_atoms" path
